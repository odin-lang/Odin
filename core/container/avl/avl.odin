...
// first returns the first node in the tree (in-order) or nil if
// the tree is empty.
first :: proc "contextless" (t: ^$T/Tree($Value)) -> ^Node(Value) {
	return tree_first_or_last_in_order(t, Direction.Backward)
}

// last returns the last element in the tree (in-order) or nil if
// the tree is empty.
...
// node or nil if the value is not present.
...
// remove removes a node or value from the tree, and returns true if the
...
// remove_value removes a value from the tree, and returns true if the
// removal was successful.  While the node's value will be left intact,
// the node itself will be freed via the tree's node allocator.
...
// remove_node removes a node from the tree, and returns true if the
// removal was successful.  While the node's value will be left intact,
// the node itself will be freed via the tree's node allocator.
...
// or nil if the node has been removed, the tree is empty, or the end
...
// and returns true if the removal was successful.  Semantics are the
...
// or (nil, false) if the end of the tree has been reached.
//
