...
- allocator: The allocator to use for the result. Default is context.allocator.
...
/*
[DEPRECATED - Prefer `clone`] Clones a string safely (returns early with an allocation error on failure).
...
*WARNING*: The created string is only valid as long as the pointer and length are valid.

Returns:
- res: A string created from the byte pointer and length.
*/
string_from_ptr :: proc(ptr: ^byte, len: int) -> (res: string) {
	// @Cleanup(ema): Maybe ptr should be a multi-pointer.
...
/*
[DEPRECATED - Prefer the builtin `raw_data`] Gets the raw byte pointer for the start of a string `str`.
...
/*
Casts a string to a cstring, assuming the presence of a null-byte at the end. Non-allocating.
...
/*
Truncates a string `str` at the first occurrence of char/byte `b`. Non-allocating, the result is a slice of the original string. If `b` isn't found, the entire string is returned.
...
/*
Truncates a string `str` at the first occurrence of rune `r`. Non-allocating, the result is a slice of the original string. If `r` isn't found, the entire string is returned.
...
/*
Clones a cstring `s` as a string, including the terminating null-byte.

*Allocates Using Provided Allocator*

Inputs:
- s: The cstring to clone.
- allocator: The allocator to use for the result. Default is context.allocator.
- loc: The caller source code location, for debugging purposes. Default is #caller_location.
...
Inputs:
- ptr: A pointer to the start of the byte sequence to clone.
- len: The length of the byte sequence.
- allocator: The allocator to use for the result. Default is context.allocator.
- loc: The caller source code location, for debugging purposes. Default is #caller_location.

NOTE: Same as `string_from_ptr`, but performs an additional `clone` operation.
...
Inputs:
- ptr: A pointer to the start of the null-terminated cstring to clone.
...
The possible results are: -1 for `lhs`; 1 for `rhs`, or 0 if they are equal.

Inputs:
- lhs, rhs: The two strings to compare.
...
/*
Checks if the string `s` contains the rune `r`.
...
/*
Checks if the string `s` contains the sequence `substr`.
...
/*
Checks if the string `s` contains any of the characters inside the string `chars`.
...
/*
Returns the UTF-8 rune count of the string `s`. It is different than the built-in `len` in that it counts multi-byte unicode characters as 1.

Inputs:
- s: The string to count the runes of.
...
/*
Checks whether the strings `u` and `v` are the same alpha characters, ignoring different casings.
Works with UTF-8 string content.

Inputs:
- u, v: The two strings to compare.

Returns:
- res: `true` if the strings `u` and `v` are the same alpha characters (ignoring case), `false` otherwise.
...
equal_fold :: proc(u, v: string) -> (res: bool) {
	// @Note(ema): Is this just compare_insensitive?
	s, t := u, v
	loop: for s != "" && t != "" {
		sr, tr: rune
...
Returns:
- result: `true` if the string `s` starts with the `prefix`, `false` otherwise.
...
Returns:
- result: `true` if the string `s` ends with the `suffix`, `false` otherwise.
...
/*
Joins a slice of strings into a single string, inserting a separator string. To join strings without a separator, see `concatenate`.

*Allocates Using Provided Allocator*

Inputs:
- a: The slice of strings to join.
...
/*
[DEPRECATED - Prefer `join`] Joins a slice of strings `a` with a separator `sep` string, returns an error on allocation failure.
...
/*
Joins a slice of strings into a single string, without a separator. To join strings with a separator, see `join`.

*Allocates Using Provided Allocator*

Inputs:
- a: The slice of strings to concatenate.
- allocator: The allocator to use for the result. Default is context.allocator.
- loc: The caller source code location, for debugging purposes. Default is #caller_location. 
...
/*
[DEPRECATED - Prefer `concatenate`] Returns a combined string from the slice of strings `a` without a separator, or an error if allocation fails.
...
Inputs:
- s: The string to cut
- rune_offset: The starting rune index, in runes (not bytes). Default is 0.
- rune_length: The number of runes to include in the substring. Default is 0, which returns the remainder of the string.
- allocator: The allocator to use for the result. Default is context.allocator.
- loc: The caller source code location, for debugging purposes. Default is #caller_location.
...
// Contains `true` for ASCII space characters ('\t', '\n', '\v', '\f', '\r', ' ')
...
Returns:
- res: `true` if `r` is a whitespace character, `false` otherwise.
*/
...
Returns:
- res: `true` if `r` is `0x0`, `false` otherwise.
*/
...
- truth: The boolean value to be matched. Default is `true`.
...
/*
Find the index of the first rune `r` in string `s` for which procedure `p` returns the same as `truth`, or -1 if no such rune appears.
It is the same as `index_proc`, but the `p` procedure also takes a raw pointer for state.

Inputs:
- s: The string to be tested.
- p: A procedure that takes a rune and a raw pointer and returns a boolean.
- truth: The boolean value to be matched. Default is `true`.
...
/*
Find the index of the last rune `r` in string `s` for which procedure `p` returns the same as `truth`, or -1 if no such rune appears.
It is the same as `index_proc`, but the string is scanned in reverse.
...
Returns:
- res: The index of the last matching rune, or -1 if no match was found.
*/
...
/*
Find the index of the last rune `r` in string `s` for which procedure `p` returns the same as `truth`, or -1 if no such rune appears.
It is the same as `last_index_proc`, but the `p` procedure also takes a raw pointer for state.
...
/*
Trims the string `s` from the right until the procedure `p` returns `false`.
...
Returns:
- res: The trimmed string as a slice of the original, or an empty string when no match occurred.
*/
...
/*
Checks if the specified rune is part of a string. It is used for `trim_*_proc_with_state` variants.

Inputs:
- state: The cutset string as a `rawptr`.
- r: The rune to be checked for.

Returns:
- res: `true` if the rune is in the cutset, `false` otherwise.
*/
...
/*
Trims the cutset string from start of the `s` string.

Inputs:
- s: The string to be trimmed.
- cutset: The set of characters to be trimmed from the left of `s`.
...
/*
Trims the `cutset` string from the end of the `s` string.

Inputs:
- s: The string to be trimmed.
- cutset: The set of characters to be trimmed from the right of `s`.
...
/*
Trims the `cutset` string from both sides of the `s` string.

Inputs:
- s: The string to be trimmed.
- cutset: The set of characters to be trimmed from both sides of `s`.
...
/*
Trims spaces from the start of the string `s` and returns the trimmed string.
...
Example:
	...
	trim_left_space_example :: proc() {
		fmt.println(strings.trim_left_space("\t\txyz\t\t")) // -> "xyz\t\t"
	}
*/
...
/*
Trims spaces from the end of the string `s` and returns the trimmed string.
...
Example:
	...
	trim_right_space_example :: proc() {
		fmt.println(strings.trim_right_space("\t\txyz\t\t")) // -> "\t\txyz"
	}
*/
...
/*
Trims spaces from both sides of the string `s` and returns the trimmed string.
...
Example:
	...
	trim_space_example :: proc() {
		fmt.println(strings.trim_space("\t\txyz\t\t")) // -> "xyz"
	}
*/
...
/*
Trims null runes from the start of the string `s` and returns the trimmed string.
...
Example:
	...
	trim_left_null_example :: proc() {
		fmt.println(strings.trim_left_null("\x00\x00testing\x00\x00")) // -> "testing\x00\x00"
	}
...
/*
Trims null runes from the end of the string `s` and returns the trimmed string.
...
Example:
	...
	trim_right_null_example :: proc() {
		fmt.println(strings.trim_right_null("\x00\x00testing\x00\x00")) // -> "\x00\x00testing"
	}
...
/*
Trims null runes from both sides of the string `s` and returns the trimmed string.
...
Example:
	...
	trim_null_example :: proc() {
		fmt.println(strings.trim_null("\x00\x00testing\x00\x00")) // -> "testing"
	}
...
Returns:
- res: The trimmed string as a slice of original, or the input string if the prefix wasn't found.
...
Returns:
- res: The trimmed string as a slice of the original, or the input string if the suffix wasn't found.
...
Example:
	...
	scrub_example :: proc() {
		...
		fmt.println(strings.scrub(text, "?")) // -> "Hello?"
	}
...
/*
Expands the input string by replacing tab characters with the necessary amout of spaces to align to a specified tab size.
...
/*
Splits the input string `str` by the separator `sep` string; returns the part of the string before the separator, the separator itself and the part after the separator. Non-allocating, all the results are slices of the original string.
...
Returns:
- head: the string before the separator.
- match: the separator string.
- tail: the string after the separator.
...
/*
Centers the input string within a field of specified length by adding pad string on both sides, if its length is less than the target length. It is an alias for `centre_justify`.
...
Example:
	...
	center_justify_example :: proc() {
		fmt.println(strings.center_justify(strings.center_justify("TITLE", 7, " "), 15 "-")
	}
...
Example:
	...
	centre_justify_example :: proc() {
		fmt.println(strings.centre_justify(strings.centre_justify("TITLE", 7, " "), 15 "-")
	}
...
Example:
	...
	left_justify_example :: proc() {
		fmt.println(strings.left_justify("3.14", 10, "0"))
	}
...
Inputs:
- str: The string to be right-justified.
...
Example:
	...
	right_justify_example :: proc() {
		fmt.println("FPS:", strings.right_justify("60", 10, " "))
	}
...
right_justify :: proc(str: string, length: int, pad: string, allocator := context.allocator) -> (res: string, err: mem.Allocator_Error) #optional_allocator_error {
	// @Note(ema): Why doesn't it take a the caller location?
	n := rune_count(str)
	if n >= length || pad == "" {
		return clone(str, allocator)
	}
...
/*
Splits a string at each instance of one or more consecutive white space characters, as defined by `unicode.is_space`.
...
/*
Splits a string at each run of unicode code points `r` satisfying the predicate `f(r)`.
...
Example:
	...
	fields_proc_example :: proc() {
		fmt.println(strings.fields_proc("hello world", strings.is_space))
		fmt.println(strings.fields_proc("3.14", strings.is_separator))
		fmt.println(strings.fields_proc("x = 4;y = 7; z = 0", is_semicolon))

		is_semicolon :: proc(r: rune) -> bool { return r == ';' }
	}
...
Inputs:
- a, b: The two strings to compare.
- allocator: The allocator to use for internal purposes. Default is context.allocator.
- loc: The caller source code location, for debugging purposes. Default is #caller_location.
...
Example:
	...
	levenshtein_distance_example :: proc() {
		fmt.println(strings.levenshtein_distance("uninformed", "uniformed"))
		fmt.println(strings.levenshtein_distance("kitten", "sitting"))
	}
