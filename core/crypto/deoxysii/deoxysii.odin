/*
package deoxysii implements the Deoxys-II-256-128 Authenticated Encryption
...
// KEY_SIZE is the Deoxys-II-256-128 key size in bytes.
KEY_SIZE :: 32
// IV_SIZE iss the Deoxys-II-256-128 IV size in bytes.
IV_SIZE :: 15 // 120-bits
// TAG_SIZE is the Deoxys-II-256-128 tag size in bytes.
TAG_SIZE :: 16
...
// Context is a keyed Deoxys-II instance.
Context :: struct {
	_ks:             [17][16]byte,
...
reset :: proc "contextless" (ctx: ^Context) {
	mem.zero_explicit(&ctx._ks, len(ctx._ks))
...
derive_ks :: proc "contextless" (ctx: ^Context, key: []byte) {
	...
	//   TK1_0 = T (Tweak)
	//   STK_0 = k_0 ^ TK1_0
	//   STK_i = k_i (precomputed) ^ H(TK1_(i-1))
	//
	...
	// k_0 does not apply LFSR2/3 or H.
	intrinsics.unaligned_store(
		(^simd.u8x16)(&ctx._ks[0]),
		simd.bit_xor(
			tk2,
			simd.bit_xor(
				tk3,
...
lfsr3 :: #force_inline proc "contextless"  (tk: simd.u8x16) -> simd.u8x16 {
	// LFSRe is a application of the following LFSR to each byte of input.
