...
// byte_eq returns 1 if a == b, 0 otherwise.
@(optimization_mode="none")
byte_eq :: proc "contextless" (a, b: byte) -> int {
	v := a ~ b

	// v == 0 if a == b.  The subtraction will underflow, setting the
	// sign bit, which will get returned.
	return int((u32(v)-1) >> 31)
}

// u64_eq returns 1 if a == b, 0 otherwise.
@(optimization_mode="none")
...
// u64_is_zero returns 1 if a == 0, 0 otherwise.
@(optimization_mode="none")
...
// u64_is_non_zero returns 1 if a != 0, 0 otherwise.
@(optimization_mode="none")
