...
ge_set_bytes :: proc "contextless" (ge: ^Group_Element, b: []byte) -> bool {
	...
	// x = invsqrt(den/num)
	is_square := field.fe_carry_sqrt_ratio_m1(&tmp.x, field.fe_relax_cast(fe_tmp), field.fe_relax_cast(&tmp.x))
	if is_square == 0 {
		return false
	}
...
ge_add :: proc "contextless" (ge, a, b: ^Group_Element) {
	...
	t4: field.Tight_Field_Element = ---
	t0, t1, t2, t3, t5: field.Loose_Field_Element = ---, ---, ---, ---, ---

	field.fe_sub(&t0, &a.y, &a.x)
	field.fe_sub(&t1, &b.y, &b.x)
	field.fe_carry_mul(&A, &t0, &t1)
	field.fe_add(&t2, &a.y, &a.x)
	field.fe_add(&t3, &b.y, &b.x)
	field.fe_carry_mul(&B, &t2, &t3)
	field.fe_carry_mul(&t4, field.fe_relax_cast(&FE_D2), field.fe_relax_cast(&b.t))
	field.fe_carry_mul(&C, field.fe_relax_cast(&a.t), field.fe_relax_cast(&t4))
	field.fe_add(&t5, &b.z, &b.z)
...
ge_double :: proc "contextless" (ge, a: ^Group_Element) {
	...
	E, F, H, t1: field.Loose_Field_Element = ---, ---, ---, ---
...
ge_equal :: proc "contextless" (a, b: ^Group_Element) -> int {
	...
	field.fe_clear(&ax_bz)
	field.fe_clear(&ay_bz)
	field.fe_clear(&bx_az)
	field.fe_clear(&by_az)
