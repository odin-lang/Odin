...
fast_float_compute_float :: proc "contextless" ($T: typeid, q: int, w: u64) -> u64 where T == f32 || T == f64 #no_bounds_check {
	...
	if power2 <= 0 {
		...
		mantissa >>= uint(-power2 + 1)
		mantissa += mantissa & 1 // round up
		mantissa >>= 1
		// Rounding up can give the smallest normal number. Then the hidden bit
		// is set, and it is also the lowest bit of the exponent.
		return mantissa
	}
