...
main :: proc() {
	w(TABLES + "riscv.encode_forms.bin", raw(&ENCODE_FORMS, size_of(ENCODE_FORMS)))
	w(TABLES + "riscv.encode_runs.bin", raw(&ENCODE_RUNS, size_of(ENCODE_RUNS)))
	w(TABLES + "riscv.entries.bin", raw(&DECODE_ENTRIES, size_of(DECODE_ENTRIES)))
