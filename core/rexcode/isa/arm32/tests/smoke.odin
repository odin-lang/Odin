...
run_smoke :: proc() {
	...
	// ---- Load/Store ----
	check("LDR imm",         .LDR,  0, 0x05100000, 0x0F700000)
	check("LDR pre",         .LDR,  1, 0x05300000, 0x0F700000)
	check("LDR post",        .LDR,  2, 0x04100000, 0x0F700000)
	check("STR imm",         .STR,  0, 0x05000000, 0x0F700000)
	check("LDRB imm",        .LDRB, 0, 0x05500000, 0x0F700000)
	check("STRH imm",        .STRH, 0, 0x014000B0, 0x0F7000F0)
	check("LDRSB imm",       .LDRSB, 0, 0x015000D0, 0x0F7000F0)
	check("LDRD",            .LDRD, 0, 0x014000D0, 0x0F7000F0)
	check("LDM",             .LDM, 0, 0x08900000, 0x0FF00000)
	check("STM DB",          .STMDB, 0, 0x09000000, 0x0FD00000)
	...
	// NEON unary + shifts
	check("VABS NEON.S8 D",  .VABS, 2, 0xF3B10300, 0xFFBF0FD0)
	check("VNEG NEON.F32 D", .VNEG, 8, 0xF3B90780, 0xFFBF0FD0)
	check("VMVN",            .VMVN, 0, 0xF3B00580, 0xFFB30FD0)
	check("VCNT",            .VCNT, 0, 0xF3B00500, 0xFFB30FD0)
	check("VCLZ.I8 D",       .VCLZ, 0, 0xF3B00480, 0xFFBF0FD0)
	...
	check("VREV64.I8 D",     .VREV64, 0, 0xF3B00000, 0xFFBF0FD0)
	check("VTRN.I8 D",       .VTRN, 0, 0xF3B20080, 0xFFBF0FD0)
	check("VZIP.I8 D",       .VZIP, 0, 0xF3B20180, 0xFFB70FD0)
	...
	check("LDM T16",         .LDM,  2, 0x0000C800, 0x0000F800)
	check("STM T16",         .STM,  2, 0x0000C000, 0x0000F800)
	...
	check("AND T32 imm",     .AND, 7,  0xF0000000, 0xFBF08000)
	check("AND T32 reg",     .AND, 9,  0xEA000000, 0xFFF08000)
	...
	check("LDM T32 IA",      .LDM, 3, 0xE8900000, 0xFFD00000)
	check("LDM T32 DB",      .LDMDB, 1, 0xE9100000, 0xFFD00000)
	check("STM T32 IA",      .STM, 3, 0xE8800000, 0xFFD00000)
	...
	check("VMOVN.I16",       .VMOVN, 0, 0xF3B20200, 0xFFBF0FD0)
	check("VQMOVN.S16",      .VQMOVN, 0, 0xF3B20280, 0xFFBF0FD0)
	check("VQMOVUN.S16",     .VQMOVUN, 0, 0xF3B20240, 0xFFBF0FD0)
	check("VSHRN imm",       .VSHRN, 0, 0xF2800810, 0xFE800FD0)
	check("VQSHRN.S",        .VQSHRN, 0, 0xF2800910, 0xFE800FD0)
	check("VPADDL.S8",       .VPADDL, 0, 0xF3B00200, 0xFFBF0FD0)
	...
	check("VSDOT lane D",    .VSDOT, 2, 0xFE200D00, 0xFFB00F10)
	...
	check("VMUL lane Q .S32", .VMUL,  21, 0xF3A00840, 0xFFB00F50)
	check("VMLA lane D",      .VMLA,  12, 0xF2900040, 0xFFB00F50)
	check("VMLS lane Q",      .VMLS,  13, 0xF3900440, 0xFFB00F50)
	check("VMULL lane .S16",  .VMULL, 7, 0xF2900A40, 0xFFB00F50)
	check("VMLAL lane .S32",  .VMLAL, 7, 0xF2A00240, 0xFFB00F50)
	check("VMLSL lane .S16",  .VMLSL, 6, 0xF2900640, 0xFFB00F50)
	...
	check("VCEQ Q .F32",   .VCEQ, 13, 0xF3B90540, 0xFFBB0FD0)
	check("VCGE D",        .VCGE, 12, 0xF3B10080, 0xFFBB0FD0)
	check("VCGT D",        .VCGT, 12, 0xF3B10000, 0xFFBB0FD0)
	check("VCLE D",        .VCLE, 14, 0xF3B10180, 0xFFBB0FD0)
