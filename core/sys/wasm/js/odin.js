...
(function () {
	...
	class WasmMemoryInterface {
		...
		setMemory(memory) {
			...
			this.isShared = typeof SharedArrayBuffer !== "undefined" && memory != null && Object.prototype.toString.call(memory.buffer) === "[object SharedArrayBuffer]";
		}
	...
	class WebGLInterface {
		...
		populateUniformTable(program) {
			...
			for (
				let ptable = this.programInfos[program], utable = ptable.uniforms, numUniforms = this.ctx.getProgramParameter(p, this.ctx.ACTIVE_UNIFORMS), i = 0;
				i < numUniforms;
				++i
			) {
	...
	function odinSetupDefaultImports(wasmMemoryInterface, consoleElement, memory) {
		...
		const writeToConsole = (line, isError) => {
			...
			const println = (text, forceIsError) => {
				let style = ["color: #eee", "background-color: #d20", "padding: 2px 4px", "border-radius: 2px"].join(";");
