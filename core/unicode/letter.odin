...
is_decimal_digit :: proc(r: rune) -> bool {
	c := i32(r);
	p := binary_search(c, decimal_digit_ranges[:], len(decimal_digit_ranges)/2, 2);
	if p >= 0 && decimal_digit_ranges[p] <= c && c <= decimal_digit_ranges[p+1] {
		return true;
	}
	return false;
}
