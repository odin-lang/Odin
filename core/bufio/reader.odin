...
// A buffered wrapper for an `io.Reader`.
Reader :: struct {
	buf:            []byte,
...
// Unreads the last rune. Only the most recently read rune can be unread.
...
//
// If `reader_read_slice` encounters an error before finding a delimiter, it returns
// with that error and a buffer containing everything it had read up to that point.
