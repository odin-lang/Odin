...
Warning_Handler :: #type proc(pos: Pos, fmt: string, args: ..any)
...
warn :: proc(p: ^Parser, pos: Pos, msg: string, args: ..any) {
	if p.warn != nil {
		p.warn(pos, msg, ..args)
	}
...
parse_file :: proc(p: ^Parser, file: ^File) -> bool {
	...
	p.file = file
	init(&p.tok, file.src, file.fullpath, p.err)
	if p.tok.ch <= 0 {
		return true
	}
	...
	pd := new(Package_Decl, pkg_name.pos, end_pos(p.prev_tok))
	pd.docs    = docs
...
peek_token_kind :: proc(p: ^Parser, kind: Token_Kind, lookahead := 0) -> (ok: bool) {
	prev_parser := p^
	p.peeking = true
...
peek_token :: proc(p: ^Parser, lookahead := 0) -> (tok: Token) {
	prev_parser := p^
	p.peeking = true
...
consume_comment_group :: proc(p: ^Parser, n: int) -> (comments: ^Comment_Group, end_line: int) {
	...
	if len(list) > 0 && !p.peeking {
		comments = new(Comment_Group, list[0].pos, end_pos(list[len(list)-1]))
		comments.list = list[:]
...
end_of_line_pos :: proc(p: ^Parser, tok: Token) -> Pos {
...
expect_closing_brace_of_field_list :: proc(p: ^Parser) -> Token {
	...
	if allow_token(p, .Semicolon) && !is_newline(token) {
...
is_non_inserted_semicolon :: proc(tok: Token) -> bool {
...
is_semicolon_optional_for_node :: proc(p: ^Parser, node: ^Node) -> bool {
	...
	case ^If_Stmt, ^When_Stmt,
	     ^For_Stmt, ^Range_Stmt, ^Inline_Range_Stmt,
	     ^Switch_Stmt, ^Type_Switch_Stmt:
		return true
	...
	case ^Struct_Type, ^Union_Type, ^Enum_Type:
		// Require semicolon within a procedure body
		return p.curr_proc == nil
	case ^Proc_Lit:
		return true

	case ^Package_Decl, ^Import_Decl, ^Foreign_Import_Decl:
		return true
...
expect_semicolon :: proc(p: ^Parser, node: ^Node) -> bool {
	...
	error(p, prev.pos, "expected ';', got %s", token_to_string(p.curr_tok))
	fix_advance_to_next_stmt(p)
	return false
}

new_blank_ident :: proc(p: ^Parser, pos: Pos) -> ^Ident {
	tok: Token
	tok.pos = pos
	i := new(Ident, pos, end_pos(tok))
	i.name = "_"
	return i
}
...
parse_when_stmt :: proc(p: ^Parser) -> ^When_Stmt {
	...
	if p.curr_tok.kind == .Else {
		...
		case:
			...
			else_stmt = new(Bad_Stmt, p.curr_tok.pos, end_pos(p.curr_tok))
		}
	}
	...
	when_stmt := new(When_Stmt, tok.pos, end)
...
convert_stmt_to_expr :: proc(p: ^Parser, stmt: ^Stmt, kind: string) -> ^Expr {
	...
	return new(Bad_Expr, p.curr_tok.pos, end_pos(p.curr_tok))
}
...
parse_if_stmt :: proc(p: ^Parser) -> ^If_Stmt {
	...
	if_stmt := new(If_Stmt, tok.pos, end)
...
parse_for_stmt :: proc(p: ^Parser) -> ^Stmt {
	...
	if p.curr_tok.kind != .Open_Brace && p.curr_tok.kind != .Do {
		...
		if p.curr_tok.kind == .In {
			...
			range_stmt := new(Range_Stmt, tok.pos, body.end)
		...
		if !is_range && parse_control_statement_semicolon_separator(p) {
			...
			if p.curr_tok.kind == .Open_Brace || p.curr_tok.kind == .Do {
				error(p, p.curr_tok.pos, "Expected ';', followed by a condition expression and post statement, got %s", tokens[p.curr_tok.kind])
			} else {
				if p.curr_tok.kind != .Semicolon {
				...
				if p.curr_tok.text != ";" {
					error(p, p.curr_tok.pos, "Expected ';', got %s", token_to_string(p.curr_tok))
				} else {
	...
	for_stmt := new(For_Stmt, tok.pos, body.end)
...
parse_case_clause :: proc(p: ^Parser, is_type_switch: bool) -> ^Case_Clause {
	...
	cc := new(Case_Clause, tok.pos, end_pos(p.prev_tok))
	cc.list = list
...
parse_switch_stmt :: proc(p: ^Parser) -> ^Stmt {
	...
	if p.curr_tok.kind != .Open_Brace {
		...
		if p.curr_tok.kind == .In {
			...
			as := new(Assign_Stmt, tok.pos, rhs[0].end)
			as.lhs = lhs
			as.op  = in_tok
			as.rhs = rhs
			tag = as
			...
			if as, ok := tag.derived.(^Assign_Stmt); ok && as.op.kind == .In {
				is_type_switch = true
	...
	body := new(Block_Stmt, open.pos, end_pos(close))
	body.stmts = clauses[:]

	if is_type_switch {
		ts := new(Type_Switch_Stmt, tok.pos, body.end)
		ts.tag  = tag
		ts.body = body
		...
		ts := new(Switch_Stmt, tok.pos, body.end)
		ts.init = init
		ts.cond = cond
		ts.body = body
		ts.switch_pos = tok.pos
...
parse_attribute :: proc(p: ^Parser, tok: Token, open_kind, close_kind: Token_Kind, docs: ^Comment_Group) -> ^Stmt {
	...
	if p.curr_tok.kind == .Ident {
		...
		for p.curr_tok.kind != close_kind &&
			...
			if p.curr_tok.kind == .Eq {
				...
				value := parse_value(p)
				fv := new(Field_Value, elem.pos, value.end)
				fv.field = elem
				fv.sep   = eq.pos
				fv.value = value
	...
	attribute := new(Attribute, tok.pos, end_pos(close))
...
parse_foreign_block_decl :: proc(p: ^Parser) -> ^Stmt {
	decl := parse_stmt(p)
	#partial switch _ in decl.derived_stmt {
	case ^Empty_Stmt, ^Bad_Stmt, ^Bad_Decl:
		// Ignore
		return nil
	case ^When_Stmt, ^Value_Decl:
		return decl
	}
...
parse_foreign_block :: proc(p: ^Parser, tok: Token) -> ^Foreign_Block_Decl {
	docs := p.lead_comment

	foreign_library: ^Expr
	#partial switch p.curr_tok.kind {
	case .Open_Brace:
		i := new(Ident, tok.pos, end_pos(tok))
		i.name = "_"
		foreign_library = i
	case:
	...
	decl := new(Foreign_Block_Decl, tok.pos, body.end)
	decl.docs            = docs
	decl.tok             = tok
...
parse_foreign_decl :: proc(p: ^Parser) -> ^Decl {
	docs := p.lead_comment
	...
	case .Import:
		...
		decl := new(Foreign_Import_Decl, tok.pos, end_pos(p.prev_tok))
		decl.docs            = docs
	...
	return new(Bad_Decl, tok.pos, end_pos(tok))
}


parse_unrolled_for_loop :: proc(p: ^Parser, inline_tok: Token) -> ^Stmt {
	...
	if bad_stmt {
		return new(Bad_Stmt, inline_tok.pos, end_pos(p.prev_tok))
	}

	range_stmt := new(Inline_Range_Stmt, inline_tok.pos, body.end)
...
parse_stmt :: proc(p: ^Parser) -> ^Stmt {
	...
	case .Defer:
		...
		ds := new(Defer_Stmt, tok.pos, stmt.end)
		ds.stmt = stmt
		return ds
	...
	case .Return:
		...
		rs := new(Return_Stmt, tok.pos, end)
		rs.results = results[:]
		return rs
	...
	case .Using:
		...
		if len(list) == 0 {
			...
			expect_semicolon(p, nil)
			return new(Bad_Stmt, tok.pos, end_pos(p.prev_tok))
		}

		if p.curr_tok.kind != .Colon {
			end := list[len(list)-1]
			expect_semicolon(p, end)
			us := new(Using_Stmt, tok.pos, end.end)
			us.list = list
			return us
		}
	...
	case .Hash:
		...
		case "partial":
			...
			case ^Type_Switch_Stmt: s.partial = true
			case: error(p, stmt.pos, "#partial can only be applied to a switch statement")
			}
			return stmt
		case "assert", "panic":
			bd := new(Basic_Directive, tok.pos, end_pos(tag))
			bd.tok  = tok
			bd.name = name
			ce := parse_call_expr(p, bd)
			es := new(Expr_Stmt, ce.pos, ce.end)
			es.expr = ce
			return es

		case "force_inline", "force_no_inline":
			expr := parse_inlining_operand(p, true, tag)
			es := new(Expr_Stmt, expr.pos, expr.end)
			es.expr = expr
			return es
		case "unroll":
		...
		case "include":
			...
			return new(Bad_Stmt, tok.pos, end_pos(tag))
		case:
			stmt := parse_stmt(p)
			te := new(Tag_Stmt, tok.pos, stmt.pos)
			te.op   = tok
			te.name = name
			te.stmt = stmt
	...
	case .Semicolon:
		tok := advance_token(p)
		s := new(Empty_Stmt, tok.pos, end_pos(tok))
		return s
	}
	...
	case .Else:
		...
		case:
			fix_advance_to_next_stmt(p)
			return new(Bad_Stmt, token.pos, end_pos(p.curr_tok))
		}
	}


	tok := advance_token(p)
	error(p, tok.pos, "expected a statement, got %s", token_to_string(tok))
	fix_advance_to_next_stmt(p)
	s := new(Bad_Stmt, tok.pos, end_pos(tok))
	return s
}
...
parse_body :: proc(p: ^Parser) -> ^Block_Stmt {
	...
	bs := new(Block_Stmt, open.pos, end_pos(close))
	bs.open = open.pos
	bs.stmts = stmts
	bs.close = close.pos
	return bs
...
convert_stmt_to_body :: proc(p: ^Parser, stmt: ^Stmt) -> ^Stmt {
	...
	bs := new(Block_Stmt, stmt.pos, stmt.end)
	bs.open = stmt.pos
...
convert_to_ident_list :: proc(p: ^Parser, list: []Expr_And_Flags, ignore_flags, allow_poly_names: bool) -> []^Expr {
	...
	for ident, i in list {
		...
		case:
			...
			id = new(Ident, ident.expr.pos, ident.expr.end)
		}
...
parse_field_prefixes :: proc(p: ^Parser) -> (flags: Field_Flags) {
	...
	for kind in Field_Flag {
		...
		if kind == .Invalid || kind == .Unknown {
			...
			if count > 1 { error(p, p.curr_tok.pos, "multiple '%s' in this field list", field_flag_strings[kind]) }
...
check_field_flag_prefixes :: proc(p: ^Parser, name_count: int, allowed_flags, set_flags: Field_Flags) -> (flags: Field_Flags) {
	flags = set_flags
	...
	for flag in Field_Flag {
		if flag not_in allowed_flags && flag in flags {
			...
			case:
				error(p, p.curr_tok.pos, "'%s' is not allowed within this field list", field_flag_strings[flag])
			}
			flags -= {flag}
		}
	}
...
parse_var_type :: proc(p: ^Parser, flags: Field_Flags) -> ^Expr {
	if .Ellipsis in flags && p.curr_tok.kind == .Ellipsis {
		...
		if type == nil {
			...
			type = new(Bad_Expr, tok.pos, end_pos(tok))
		}
		e := new(Ellipsis, type.pos, type.end)
		e.expr = type
		return e
	}
	type: ^Expr
	if .Typeid_Token in flags && p.curr_tok.kind == .Typeid {
		...
		ti := new(Typeid_Type, tok.pos, end)
		ti.tok = tok.kind
...
check_procedure_name_list :: proc(p: ^Parser, names: []^Expr) -> bool {
	if len(names) == 0 {
		return false
	}
...
parse_ident_list :: proc(p: ^Parser, allow_poly_names: bool) -> []^Expr {
	...
	for {
		if allow_poly_names && p.curr_tok.kind == .Dollar {
			...
			poly_name := new(Poly_Type, tok.pos, ident.end)
...
parse_field_list :: proc(p: ^Parser, follow: Token_Kind, allowed_flags: Field_Flags) -> (field_list: ^Field_List, total_name_count: int) {
	handle_field :: proc(p: ^Parser,
	                     seen_ellipsis: ^bool, fields: ^[dynamic]^Field,
	                     docs: ^Comment_Group,
	                     names: []^Expr,
	                     allowed_flags, set_flags: Field_Flags,
	                     ) -> bool {
		...
		is_type_ellipsis :: proc(type: ^Expr) -> bool {
			if type == nil {
				return false
			}
	...
	if p.curr_tok.kind != .Colon {
		for eaf in list {
			...
			names := make([]^Expr, 1)
			names[0] = new(Ident, tok.pos, end_pos(tok))
	...
	field_list = new(Field_List, start_tok.pos, p.curr_tok.pos)
	field_list.list = fields[:]
	return
}


parse_results :: proc(p: ^Parser) -> (list: ^Field_List, diverging: bool) {
	...
	if p.curr_tok.kind != .Open_Paren {
		...
		list = new(Field_List, field.pos, field.end)
...
parse_proc_type :: proc(p: ^Parser, tok: Token) -> ^Proc_Type {
	...
	expect_token(p, .Open_Paren)
	params, _ := parse_field_list(p, .Close_Paren, Field_Flags_Signature_Params)
...
parse_inlining_operand :: proc(p: ^Parser, lhs: bool, tok: Token) -> ^Expr {
	...
	#partial switch e in strip_or_return_expr(expr).derived_expr {
	case ^Proc_Lit:
	...
	case:
		...
		return new(Bad_Expr, tok.pos, expr.end)
	}
	return expr
}
...
parse_operand :: proc(p: ^Parser, lhs: bool) -> ^Expr {
	...
	case .Undef:
		...
		undef := new(Undef, tok.pos, end_pos(tok))
		undef.tok = tok.kind
		return undef

	case .Context:
		tok := expect_token(p, .Context)
		ctx := new(Implicit, tok.pos, end_pos(tok))
		ctx.tok = tok
		return ctx
	...
	case .Integer, .Float, .Imag,
	     .Rune, .String:
	     tok := advance_token(p)
	     bl := new(Basic_Lit, tok.pos, end_pos(tok))
	     bl.tok = tok
	     return bl
	...
	case .Open_Paren:
		...
		pe := new(Paren_Expr, open.pos, end_pos(close))
		pe.open  = open.pos
		pe.expr  = expr
		pe.close = close.pos
		return pe
	...
	case .Distinct:
		tok := advance_token(p)
		type := parse_type(p)
		dt := new(Distinct_Type, tok.pos, type.end)
		dt.tok  = tok.kind
		dt.type = type
		return dt
	...
	case .Hash:
		...
		case "type":
			type := parse_type(p)
			hp := new(Helper_Type, tok.pos, type.end)
			hp.tok  = tok.kind
			hp.type = type
			return hp

		case "file", "line", "procedure", "caller_location":
			bd := new(Basic_Directive, tok.pos, end_pos(name))
			bd.tok  = tok
			bd.name = name.text
			return bd
		...
		case "soa":
			...
			case ^Array_Type:         t.tag = bd
			case ^Dynamic_Array_Type: t.tag = bd
			case ^Pointer_Type:       t.tag = bd
			case:
		...
		case "partial":
			tag := new(Basic_Directive, tok.pos, end_pos(name))
			tag.tok = tok
			tag.name = name.text
		...
		case "relative":
			...
			rt := new(Relative_Type, tok.pos, type.end)
			rt.tag = tag_call
			rt.type = type
			return rt
		...
		case:
			...
			te := new(Tag_Expr, tok.pos, expr.pos)
			te.op   = tok
			te.name = name.text
			te.expr = expr
			return te
	...
	case .Proc:
		...
		if p.curr_tok.kind == .Open_Brace {
			...
			pg := new(Proc_Group, tok.pos, end_pos(close))
			pg.tok   = tok
			pg.open  = open.pos
			pg.args  = args[:]
			pg.close = close.pos
		...
		pl := new(Proc_Lit, tok.pos, end_pos(p.prev_tok))
		pl.type = type
		pl.body = body
		pl.tags = tags
	...
	case .Typeid:
		tok := advance_token(p)
		ti := new(Typeid_Type, tok.pos, end_pos(tok))
		ti.tok = tok.kind
	...
	case .Pointer:
		...
		elem := parse_type(p)
		ptr := new(Pointer_Type, tok.pos, elem.end)
		ptr.pointer = tok.pos
		ptr.elem = elem
		return ptr
	...
	case .Open_Bracket:
		...
		case .Pointer:
			...
			elem := parse_type(p)
			t := new(Multi_Pointer_Type, open.pos, elem.end)
			t.open = open.pos
			t.pointer = tok.pos
			t.close = close.pos
			t.elem = elem
		...
		case .Dynamic:
			...
			close := expect_token(p, .Close_Bracket)
			elem := parse_type(p)
			da := new(Dynamic_Array_Type, open.pos, elem.end)
			da.open = open.pos
		...
		case .Question:
			...
			q := new(Unary_Expr, tok.pos, end_pos(tok))
			q.op = tok
			count = q
		case:
			p.expr_level += 1
		...
		close := expect_token(p, .Close_Bracket)
		elem := parse_type(p)
		at := new(Array_Type, open.pos, elem.end)
		at.open  = open.pos
		at.len   = count
		at.close = close.pos
		at.elem  = elem
	...
	case .Map:
		...
		mt := new(Map_Type, tok.pos, value.end)
		mt.tok_pos = tok.pos
		mt.key = key
		mt.value = value
		return mt
	...
	case .Struct:
		...
		fields, name_count = parse_field_list(p, .Close_Brace, Field_Flags_Struct)
		close := expect_closing_brace_of_field_list(p)

		st := new(Struct_Type, tok.pos, end_pos(close))
	...
	case .Union:
		...
		variants: [dynamic]^Expr
		for p.curr_tok.kind != .Close_Brace && p.curr_tok.kind != .EOF {
			type := parse_type(p)
			if _, ok := type.derived.(^Bad_Expr); !ok {
		...
		ut := new(Union_Type, tok.pos, end_pos(close))
	...
	case .Enum:
		...
		et := new(Enum_Type, tok.pos, end_pos(close))
	...
	case .Bit_Set:
		...
		bst := new(Bit_Set_Type, tok.pos, end_pos(close))
		bst.tok_pos = tok.pos
		bst.open = open.pos
		bst.elem = elem
	...
	case .Matrix:
		...
		mt := new(Matrix_Type, tok.pos, elem.end)
		mt.tok_pos = tok.pos
	...
	case .Asm:
		...
		dialect := Inline_Asm_Dialect.Default
		for allow_token(p, .Hash) {
			if p.curr_tok.kind == .Ident {
		...
		e := new(Inline_Asm_Expr, tok.pos, end_pos(close))
		e.tok                = tok
...
parse_literal_value :: proc(p: ^Parser, type: ^Expr) -> ^Comp_Lit {
	...
	lit := new(Comp_Lit, pos, end_pos(close))
	lit.type  = type
	lit.open  = open.pos
	lit.elems = elems
	lit.close = close.pos
...
parse_call_expr :: proc(p: ^Parser, operand: ^Expr) -> ^Expr {
	...
	for p.curr_tok.kind != .Close_Paren &&
		...
		if p.curr_tok.kind == .Eq {
			...
			value := parse_value(p)
			fv := new(Field_Value, arg.pos, value.end)
			fv.field = arg
			fv.sep   = eq.pos
			fv.value = value
	...
	ce := new(Call_Expr, operand.pos, end_pos(close))
	ce.expr     = operand
	ce.open     = open.pos
	ce.args     = args[:]
	...
	if se, ok := o.derived.(^Selector_Expr); ok && se.op.kind == .Arrow_Right {
		sce := new(Selector_Call_Expr, ce.pos, ce.end)
		sce.expr = o
		sce.call = ce
		return sce
	}
...
parse_atom_expr :: proc(p: ^Parser, value: ^Expr, lhs: bool) -> (operand: ^Expr) {
	...
	if operand == nil {
		...
		fix_advance_to_next_stmt(p)
		be := new(Bad_Expr, p.curr_tok.pos, end_pos(p.curr_tok))
		operand = be
	}
	...
	for loop {
		...
		case .Open_Bracket:
			...
			if is_slice_op {
				if interval.kind == .Comma {
					...
					se := new(Matrix_Index_Expr, operand.pos, end_pos(close))
					se.expr = operand
					se.open = open.pos
					...
					se := new(Slice_Expr, operand.pos, end_pos(close))
					se.expr = operand
					se.open = open.pos
					se.low = indices[0]
				...
				ie := new(Index_Expr, operand.pos, end_pos(close))
				ie.expr = operand
				ie.open = open.pos
				ie.index = indices[0]
				ie.close = close.pos
		...
		case .Period:
			...
			case .Ident:
				...
				sel := new(Selector_Expr, operand.pos, field.end)
				sel.expr  = operand
				sel.op = tok
				sel.field = field
			...
			case .Open_Paren:
				...
				ta := new(Type_Assertion, operand.pos, end_pos(close))
				ta.expr  = operand
				ta.open  = open.pos
				ta.type  = type
				ta.close = close.pos
			...
			case .Question:
				...
				type := new(Unary_Expr, question.pos, end_pos(question))
				type.op = question
				type.expr = nil

				ta := new(Type_Assertion, operand.pos, type.end)
				ta.expr  = operand
				ta.type  = type
			...
			case:
				...
				advance_token(p)
				operand = new(Bad_Expr, operand.pos, end_pos(tok))
			}
		...
		case .Pointer:
			...
			deref := new(Deref_Expr, operand.pos, end_pos(op))
			deref.expr = operand
			deref.op   = op
		...
		case .Or_Return:
			...
			oe := new(Or_Return_Expr, operand.pos, end_pos(token))
			oe.expr  = operand
			oe.token = token
		...
		case .Or_Break, .Or_Continue:
			...
			oe := new(Or_Branch_Expr, operand.pos, end)
			oe.expr  = operand
			oe.token = token
			oe.label = label
...
parse_unary_expr :: proc(p: ^Parser, lhs: bool) -> ^Expr {
	...
	case .Transmute, .Cast:
		...
		tc := new(Type_Cast, tok.pos, expr.end)
		tc.tok   = tok
		tc.open  = open.pos
		tc.type  = type
		tc.close = close.pos
	...
	case .Auto_Cast:
		...
		ac := new(Auto_Cast, op.pos, expr.end)
		ac.op   = op
		ac.expr = expr
		return ac
	...
	case .Add, .Sub,
		...
		ue := new(Unary_Expr, op.pos, expr.end)
		ue.op   = op
		ue.expr = expr
		return ue
	...
	case .Period:
		op := advance_token(p)
		field := parse_ident(p)
		ise := new(Implicit_Selector_Expr, op.pos, field.end)
		ise.field = field
		return ise
...
parse_binary_expr :: proc(p: ^Parser, lhs: bool, prec_in: int) -> ^Expr {
	...
	if expr == nil {
		return new(Bad_Expr, start_pos, end_pos(p.prev_tok))
	}
	...
	for prec := token_precedence(p, p.curr_tok.kind); prec >= prec_in; prec -= 1 {
		loop: for {
			...
			case .Question:
				...
				y := parse_expr(p, lhs)
				te := new(Ternary_If_Expr, expr.pos, end_pos(p.prev_tok))
				te.cond = cond
				te.op1  = op
				te.x    = x
				te.op2  = colon
			...
			case .When:
				...
				else_tok := expect_token(p, .Else)
				y := parse_expr(p, lhs)
				te := new(Ternary_When_Expr, expr.pos, end_pos(p.prev_tok))
				te.x    = x
				te.op1  = op
				te.cond = cond
				te.op2  = else_tok
			...
			case .Or_Else:
				x := expr
				y := parse_expr(p, lhs)
				oe := new(Or_Else_Expr, expr.pos, end_pos(p.prev_tok))
				oe.x     = x
				oe.token = op
				oe.y     = y
			...
			case:
				...
				be := new(Binary_Expr, expr.pos, end_pos(p.prev_tok))
				be.left  = expr
				be.op    = op
				be.right = right
...
parse_simple_stmt :: proc(p: ^Parser, flags: Stmt_Allow_Flags) -> ^Stmt {
	...
	case is_assignment_operator(op.kind):
		...
		// 	return new(Bad_Stmt, start_tok.pos, end_pos(p.curr_tok));
		// }
		advance_token(p)
		...
		stmt := new(Assign_Stmt, lhs[0].pos, rhs[len(rhs)-1].end)
		stmt.lhs = lhs
		stmt.op = op
		stmt.rhs = rhs
		return stmt
		...
		if .Label in flags && len(lhs) == 1 {
			...
			case .Open_Brace, .If, .For, .Switch:
				...
				if stmt != nil {
					...
					case ^Type_Switch_Stmt: n.label = label
	...
	es := new(Expr_Stmt, lhs[0].pos, lhs[0].end)
	es.expr = lhs[0]
	return es
}
...
parse_value_decl :: proc(p: ^Parser, names: []^Expr, docs: ^Comment_Group) -> ^Decl {
	...
	if is_mutable {
		if type == nil && len(values) == 0 {
			...
			return new(Bad_Decl, names[0].pos, end_pos(p.curr_tok))
		}
	} else {
	...
	decl := new(Value_Decl, names[0].pos, end_pos(p.prev_tok))
	decl.docs = docs
	decl.names = names
	decl.type = type
	decl.values = values
...
parse_import_decl :: proc(p: ^Parser, kind := Import_Decl_Kind.Standard) -> ^Import_Decl {
	...
	decl := new(Import_Decl, tok.pos, end_pos(path))
	decl.docs       = docs
