...
//
// The sole exception is if `skip_dir` is returned as `true`: <br>
//     when `skip_dir` is invoked on a directory, `walk` skips directory contents <br>
//     when `skip_dir` is invoked on a non-directory, `walk` skips the remaining files in the containing directory <br>
...
//
// NOTE: Walking large directories can be inefficient due to the lexical sort. <br>
// NOTE: `walk` does not follow symbolic links. <br>
// NOTE: `os.File_Info` uses the `context.temp_allocator` to allocate, and will delete when it is done. <br>
