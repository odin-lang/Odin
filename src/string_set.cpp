...
static gb_inline void string_set_init(StringSet *s, gbAllocator a, isize capacity) {
...
static gb_internal MapIndex string_set__add_entry(StringSet *s, StringHashKey const &key) {
	StringSetEntry e = {};
	e.hash = key.hash;
	e.next = MAP_SENTINEL;
	e.value = key.string;
...
static gb_internal MapFindResult string_set__find(StringSet *s, StringHashKey const &key) {
...
static gb_internal MapFindResult string_set__find_from_entry(StringSet *s, StringSetEntry *e) {
	MapFindResult fr = {MAP_SENTINEL, MAP_SENTINEL, MAP_SENTINEL};
	if (s->hashes.count > 0) {
...
static gb_internal b32 string_set__full(StringSet *s) {
...
static gb_inline bool string_set_exists(StringSet *s, String const &str) {
