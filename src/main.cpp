...
struct BuildFlag {
	...
	u64                command_support;
	bool               allow_multiple;
	bool               optional_param; // param_kind may be omitted; the flag then carries no value
};


gb_internal void add_flag(Array<BuildFlag> *build_flags, BuildFlagKind kind, String name, BuildFlagParamKind param_kind, u64 command_support, bool allow_multiple=false, bool optional_param=false) {
	BuildFlag flag = {kind, name, param_kind, command_support, allow_multiple, optional_param};
...
gb_internal bool parse_build_flags(Array<String> args) {
	...
	add_flag(&build_flags, BuildFlag_Cached,                  str_lit("cached"),                    BuildFlagParam_String,  Command__does_build | Command_test, false, true);
		...
		for (BuildFlag const &bf : build_flags) {
			if (bf.name == name) {
				...
				if (set_flags[bf.kind]) {
					...
					if (ok) {
						if (!(bf.optional_param && value.kind == ExactValue_Invalid)) switch (bf.param_kind) {
						...
						case BuildFlag_Cached:
							...
							if (value.kind == ExactValue_Invalid) {
								build_context.cache_check_mtime = true;
							} else {
								String_Iterator it = {value.value_string, 0};
								while (it.pos < it.str.len) {
									String opt = string_trim_whitespace(string_split_iterator(&it, ','));
									if (opt == "mtime") {
										build_context.cache_check_mtime = true;
									} else if (opt == "args") {
										build_context.cache_check_args = true;
									} else if (opt == "env") {
										build_context.cache_check_env = true;
									} else if (opt.len != 0) {
										gb_printf_err("Invalid -cached option '%.*s', expected any of: mtime, args, env\n", LIT(opt));
										bad_flags = true;
									}
								}
							}
...
gb_internal int print_show_help(String const arg0, String command, String optional_flag = {}) {
	...
	if (test_only || run_or_build) {
		...
		if (print_flag("-cached:<options>")) {
			print_usage_line(2, "Cache the built executable and reuse it on the next build if nothing relevant has changed.");
			print_usage_line(2, "Stored in $ODIN_CACHE_DIR, else $XDG_CACHE_HOME/odin, else ~/.cache/odin.");
			print_usage_line(2, "Without options, only source file modification times are checked.");
			print_usage_line(2, "<options> is a comma-separated list selecting what invalidates the cache:");
			print_usage_line(3, "mtime   source file modification times (the default)");
			print_usage_line(3, "args    the set of build flags");
			print_usage_line(3, "env     the environment variables");
			print_usage_line(2, "Example: -cached:mtime,args,env");
		}
	}
