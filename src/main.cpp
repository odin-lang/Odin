						...
						case BuildFlag_DumpAstOpts:
							{
								...
								for (;;) {
									...
									if (opt == "omit-defaults") {
										...
										if (format_set) {
											gb_printf_err("-%.*s dump format was already set\n", LIT(name));
											bad_flags = true;
											continue;
										}
										format_set = true;
										build_context.dump_ast_format = DumpAstFormat_XML;
									} else if (opt == "json") {
										if (format_set) {
											gb_printf_err("-%.*s dump format was already set\n", LIT(name));
											bad_flags = true;
										...
										format_set = true;
										build_context.dump_ast_format = DumpAstFormat_JSON;
									} else if (opt == "sexp") {
										if (format_set) {
											gb_printf_err("-%.*s dump format was already set\n", LIT(name));
											bad_flags = true;
										...
										format_set = true;
										build_context.dump_ast_format = DumpAstFormat_SEXP;
									} else if (opt == "disregard-filetags") {
										build_context.dump_ast_disregard_filetags = true;
									} else if (opt == "no-ids") {
										build_context.dump_ast_no_ids = true;
									} else {
	...
	if (run_or_build) {
		...
		if (print_flag("-dump-ast-opts:<string>")) {
			...
			print_usage_line(2, "Possible options:");
				print_usage_line(3, "sexp - dump AST in S-expression format");
				print_usage_line(3, "json - dump AST in JSON format");
				print_usage_line(3, "xml - dump AST in XML format");
				...
				print_usage_line(3, "no-ids - do not print IDs for dumped AST nodes, useful for less-cluttered diffs");
		}
	}
