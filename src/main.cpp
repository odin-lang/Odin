...
static bool global_thread_pool_add_task(WorkerTaskProc *proc, void *data) {
	return thread_pool_add_task(&global_thread_pool, proc, data);
}
static void global_thread_pool_wait(void) {
...
static i32 linker_stage(lbGenerator *gen) {
	i32 result = 0;
...
static void add_flag(Array<BuildFlag> *build_flags, BuildFlagKind kind, String name, BuildFlagParamKind param_kind, u32 command_support, bool allow_mulitple=false) {
...
static void print_show_help(String const arg0, String const &command) {
...
static int main(int arg_count, char const **arg_ptr) {
	if (arg_count < 2) {
