...
struct AstDumper {
	...
	void (*begin_dump)(AstDumper *dumper, String name);
	...
	void (*add_attribute)(AstDumper *dumper, String name, String value);
	void (*add_inline_value)(AstDumper *dumper, String name, String value);
	void (*open_ast_and_begin_attributes)(AstDumper *dumper, String name);
	void (*end_attributes_and_close_ast)(AstDumper *dumper, String name);
	void (*end_attributes)(AstDumper *dumper, String name);
	void (*close_ast)(AstDumper *dumper, String name);
};
...
gb_internal void ast_dumper_sexp_begin_dump(AstDumper *dumper, String name) {
	WRITE_LIT_STR(dumper, "(");
	WRITE_STRING(dumper, name);
	WRITE_LIT_STR(dumper, " (");
	dumper->indent++;
}
...
gb_internal void ast_dumper_sexp_add_attribute(AstDumper *dumper, String name, String value) {
	sexp_new_line(dumper);
	WRITE_LIT_STR(dumper, "(");
	WRITE_STRING(dumper, name);
	WRITE_LIT_STR(dumper, " \"");
...
gb_internal void ast_dumper_sexp_open_ast_and_begin_attributes(AstDumper *dumper, String name) {
	sexp_new_line(dumper);
	WRITE_LIT_STR(dumper, "(ast \"");
	dump_sexp_string(dumper, name);
	WRITE_LIT_STR(dumper, "\"");
	dumper->indent++;
	dumper->sexp_attributes = true;
}

gb_internal void ast_dumper_sexp_end_attributes_and_close_ast(AstDumper *dumper, String name) {
	dumper->sexp_attributes = false;
	WRITE_LIT_STR(dumper, ")");
	dumper->indent--;
}

gb_internal void ast_dumper_sexp_end_attributes(AstDumper *dumper, String name) {
	dumper->sexp_attributes = false;
}

gb_internal void ast_dumper_sexp_close_ast(AstDumper *dumper, String name) {
	WRITE_LIT_STR(dumper, ")");
	dumper->indent--;
}
...
gb_internal void ast_dumper_json_begin_dump(AstDumper *dumper, String name) {
	WRITE_LIT_STR(dumper, "{");
	dumper->indent++;
...
gb_internal void ast_dumper_json_add_attribute(AstDumper *dumper, String name, String value) {
	ast_dumper_json_start_new_line(dumper);
	WRITE_LIT_STR(dumper, "\"");
	dump_json_value(dumper, name);
	WRITE_LIT_STR(dumper, "\": \"");
...
gb_internal void ast_dumper_json_open_ast_and_begin_attributes(AstDumper *dumper, String name) {
	ast_dumper_json_start_new_line(dumper);
	WRITE_LIT_STR(dumper, "{");
	dumper->indent++;
	dumper->json_start_line = JSON_WithJustNewLine;
	ast_dumper_json_add_attribute(dumper, STR_LIT("ast-name"), name);
}

gb_internal void ast_dumper_json_end_attributes_and_close_ast(AstDumper *dumper, String name) {
	dumper->indent--;
	WRITE_LIT_STR(dumper, "\n");
	write_indent(dumper, dumper->indent);
	WRITE_LIT_STR(dumper, "}");
...
gb_internal void ast_dumper_json_end_attributes(AstDumper *dumper, String name) {
	// Nothing to do.
}

gb_internal void ast_dumper_json_close_ast(AstDumper *dumper, String name) {
	ast_dumper_json_end_attributes_and_close_ast(dumper, name);
}
...
gb_internal void ast_dumper_xml_begin_dump(AstDumper *dumper, String name) {
...
gb_internal void ast_dumper_xml_add_attribute(AstDumper *dumper, String name, String value) {
	WRITE_LIT_STR(dumper, "\n");
	write_indent(dumper, dumper->indent);
	dump_xml_name(dumper, name);
	WRITE_LIT_STR(dumper, "=\"");
...
gb_internal void ast_dumper_xml_open_ast_and_begin_attributes(AstDumper *dumper, String name) {
	WRITE_LIT_STR(dumper, "\n");
	write_indent(dumper, dumper->indent);
	WRITE_LIT_STR(dumper, "<");
	dump_xml_name(dumper, name);
...
gb_internal void ast_dumper_xml_end_attributes_and_close_ast(AstDumper *dumper, String name) {
	ast_dumper_xml_before_attributes_end(dumper);
	WRITE_LIT_STR(dumper, "/>");
	dumper->indent--;
}

gb_internal void ast_dumper_xml_end_attributes(AstDumper *dumper, String name) {
	ast_dumper_xml_before_attributes_end(dumper);
	WRITE_LIT_STR(dumper, ">");
}

gb_internal void ast_dumper_xml_close_ast(AstDumper *dumper, String name) {
	dumper->indent--;
	WRITE_LIT_STR(dumper, "\n");
	write_indent(dumper, dumper->indent);
	WRITE_LIT_STR(dumper, "</");
...
gb_internal void ast_dumper_init(AstDumper *dumper, bool omit_defaults, bool no_ids, DumpAstFormat format) {
	...
	case DumpAstFormat_SEXP:
		...
		dumper->add_attribute = ast_dumper_sexp_add_attribute;
		dumper->add_inline_value = ast_dumper_sexp_add_inline_value;
		dumper->open_ast_and_begin_attributes = ast_dumper_sexp_open_ast_and_begin_attributes;
		dumper->end_attributes_and_close_ast = ast_dumper_sexp_end_attributes_and_close_ast;
		dumper->end_attributes = ast_dumper_sexp_end_attributes;
		dumper->close_ast = ast_dumper_sexp_close_ast;
		break;
	case DumpAstFormat_JSON:
		...
		dumper->add_attribute = ast_dumper_json_add_attribute;
		dumper->add_inline_value = ast_dumper_json_add_inline_value;
		dumper->open_ast_and_begin_attributes = ast_dumper_json_open_ast_and_begin_attributes;
		dumper->end_attributes_and_close_ast = ast_dumper_json_end_attributes_and_close_ast;
		dumper->end_attributes = ast_dumper_json_end_attributes;
		dumper->close_ast = ast_dumper_json_close_ast;
		break;
	case DumpAstFormat_XML:
		...
		dumper->add_attribute = ast_dumper_xml_add_attribute;
		dumper->add_inline_value = ast_dumper_xml_add_inline_value;
		dumper->open_ast_and_begin_attributes = ast_dumper_xml_open_ast_and_begin_attributes;
		dumper->end_attributes_and_close_ast = ast_dumper_xml_end_attributes_and_close_ast;
		dumper->end_attributes = ast_dumper_xml_end_attributes;
		dumper->close_ast = ast_dumper_xml_close_ast;
		break;
	default:
		GB_PANIC("unreachable");
	}
...
gb_internal void ast_dumper_begin_dump(AstDumper *dumper, String name) {
  dumper->begin_dump(dumper, name);
}
...
gb_internal void ast_dumper_add_attribute(AstDumper *dumper, String name, String value) {
  dumper->add_attribute(dumper, name, value);
}
...
gb_internal void ast_dumper_open_ast_and_begin_attributes(AstDumper *dumper, String name) {
	dumper->open_ast_and_begin_attributes(dumper, name);
}

gb_internal void ast_dumper_end_attributes_and_close_ast(AstDumper *dumper, String name) {
	dumper->end_attributes_and_close_ast(dumper, name);
}

gb_internal void ast_dumper_end_attributes(AstDumper *dumper, String name) {
	dumper->end_attributes(dumper, name);
}

gb_internal void ast_dumper_close_ast(AstDumper *dumper, String name) {
	dumper->close_ast(dumper, name);
}
...
gb_internal void ast_dumper_dump_token(AstDumper *dumper, String token_name, Token const& token) {
	...
	ast_dumper_add_attribute(dumper, token_name, token.string);
}
...
gb_internal void ast_dumper_dump_bool(AstDumper *dumper, String name, bool value, bool default_value = false) {
	...
	ast_dumper_add_attribute(dumper, name, string_value);
}
...
gb_internal void ast_dumper_dump_proc_inlining(AstDumper *dumper, ProcInlining inlining) {
	...
	ast_dumper_add_attribute(dumper, STR_LIT("inlining"), string_value);
}
...
gb_internal void ast_dumper_dump_proc_tailing(AstDumper *dumper, ProcTailing tailing) {
	...
	ast_dumper_add_attribute(dumper, STR_LIT("tailing"), string_value);
}
...
gb_internal void ast_dumper_dump_ast(AstDumper *dumper, Ast *ast) {
	{
		...
		if (saved_id != nullptr) {
			...
			// expressions
			ast_dumper_open_ast_and_begin_attributes(dumper, STR_LIT("already_visited"));
			if (!dumper->no_ids) {
				// u32max is 10 digits
				char buf[11];
				gb_u64_to_str(*saved_id, buf, 10);
				ast_dumper_add_attribute(dumper, STR_LIT("id"), make_string(cast(u8 *)buf, -1));
			}
			ast_dumper_end_attributes_and_close_ast(dumper, STR_LIT("already_visited"));
			return;
		}
	}
	String name = ast_strings[ast->kind];
	{
		ast_dumper_open_ast_and_begin_attributes(dumper, name);
		if (!dumper->no_ids) {
			// u32max is 10 digits
			char buf[11];
			gb_u64_to_str(dumper->ast_to_id.count, buf, 10);
			ast_dumper_add_attribute(dumper, STR_LIT("id"), make_string(cast(u8 *)buf, -1));
		}
	...
	case Ast_SelectorExpr:
		...
		{
			char buf[2];
			String bufstr = make_string(cast(u8 *)buf, 1);
			if (ast->SelectorExpr.swizzle_indices != 0) {
				gb_u64_to_str(((ast->SelectorExpr.swizzle_indices >> 0) & 0x3), buf, 10);
				GB_ASSERT(buf[1] == '\0');
				ast_dumper_add_attribute(dumper, STR_LIT("swizzle_index_0"), bufstr);
				gb_u64_to_str(((ast->SelectorExpr.swizzle_indices >> 2) & 0x3), buf, 10);
				GB_ASSERT(buf[1] == '\0');
				ast_dumper_add_attribute(dumper, STR_LIT("swizzle_index_1"), bufstr);
				gb_u64_to_str(((ast->SelectorExpr.swizzle_indices >> 4) & 0x3), buf, 10);
				GB_ASSERT(buf[1] == '\0');
				ast_dumper_add_attribute(dumper, STR_LIT("swizzle_index_2"), bufstr);
				gb_u64_to_str(((ast->SelectorExpr.swizzle_indices >> 6) & 0x3), buf, 10);
				GB_ASSERT(buf[1] == '\0');
				ast_dumper_add_attribute(dumper, STR_LIT("swizzle_index_3"), bufstr);
			}
		}
	...
	case Ast_InlineAsmExpr:
		...
		ast_dumper_dump_bool(dumper, STR_LIT("is_align_stack"), ast->InlineAsmExpr.is_align_stack);
		{
			if (!dumper->omit_defaults || (ast->InlineAsmExpr.dialect != InlineAsmDialect_Default)) {
				...
				ast_dumper_add_attribute(dumper, STR_LIT("dialect"), dialect_string);
			}
		}
		ast_dumper_end_attributes(dumper, name);
	...
	case Ast_ProcType:
		...
		ast_dumper_dump_proc_tags(dumper, ast->ProcType.tags);
		{
			...
			ast_dumper_add_attribute(dumper, STR_LIT("calling_convention"), cc_string);
		}
	...
	case Ast_StructType:
		...
		{
			char buf[21];
			gb_u64_to_str(ast->StructType.field_count, buf, 10);
			ast_dumper_add_attribute(dumper, STR_LIT("field_count"), make_string(cast(u8 *)buf, -1));
		}
	...
	case Ast_UnionType:
		ast_dumper_dump_token(dumper, STR_LIT("token"), ast->UnionType.token);
		{
			if (!dumper->omit_defaults || (ast->UnionType.kind != UnionType_Normal)) {
				...
				ast_dumper_add_attribute(dumper, STR_LIT("kind"), kind_string);
			}
		}
	...
	if (!closed) {
		ast_dumper_close_ast(dumper, ast_strings[ast->kind]);
	}
}

gb_internal void ast_dumper_dump_file_tags(AstDumper *dumper, Array<Token> const& tags) {
	ast_dumper_open_ast_and_begin_attributes(dumper, STR_LIT("file tags"));
	...
	ast_dumper_close_ast(dumper, STR_LIT("file tags"));
}
