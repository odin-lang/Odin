...
gb_internal void ast_dumper_dump_ast(AstDumper *dumper, Ast *ast) {
	...
	case Ast_BasicDirective:
		ast_dumper_dump_token(dumper, STR_LIT("token"), ast->BasicDirective.token);
		ast_dumper_dump_token(dumper, STR_LIT("name"), ast->BasicDirective.name);
		break;
	case Ast_Ellipsis:
	...
	case Ast_ProcGroup:
		...
		break;
	case Ast_AsmGroup:
		ast_dumper_dump_token(dumper, STR_LIT("token"), ast->AsmGroup.token);
		ast_dumper_dump_token(dumper, STR_LIT("open"), ast->AsmGroup.open);
		ast_dumper_dump_named_slice_of_asts(dumper, STR_LIT("args"), ast->AsmGroup.args);
		ast_dumper_dump_token(dumper, STR_LIT("close"), ast->AsmGroup.close);
		break;
	case Ast_ProcLit:
	...
	case Ast_AsmTemplate:
		ast_dumper_dump_token(dumper, STR_LIT("token"), ast->AsmTemplate.token);
		ast_dumper_dump_named_ast(dumper, STR_LIT("signature"), ast->AsmTemplate.signature);
		ast_dumper_dump_named_slice_of_asts(dumper, STR_LIT("specs"), ast->AsmTemplate.specs);
		ast_dumper_dump_named_slice_of_asts(dumper, STR_LIT("clobbers"), ast->AsmTemplate.clobbers);
		ast_dumper_dump_named_slice_of_asts(dumper, STR_LIT("instructions"), ast->AsmTemplate.instructions);
		ast_dumper_dump_token(dumper, STR_LIT("end"), ast->AsmTemplate.end);
		break;
	case Ast_AsmRegister:
		ast_dumper_dump_token(dumper, STR_LIT("token"), ast->AsmRegister.token);
		ast_dumper_dump_token(dumper, STR_LIT("name"), ast->AsmRegister.name);
		ast_dumper_dump_token(dumper, STR_LIT("flag"), ast->AsmRegister.flag);
		break;
	case Ast_AsmSpec:
		ast_dumper_dump_named_ast(dumper, STR_LIT("name"), ast->AsmSpec.name);
		ast_dumper_dump_named_ast(dumper, STR_LIT("tied_name"), ast->AsmSpec.tied_name);
		ast_dumper_dump_named_ast(dumper, STR_LIT("type"), ast->AsmSpec.type);
		ast_dumper_dump_named_ast(dumper, STR_LIT("value"), ast->AsmSpec.value);
		ast_dumper_dump_named_slice_of_asts(dumper, STR_LIT("directives"), slice_from_array(ast->AsmSpec.directives));
		break;
	case Ast_AsmClobber:
		ast_dumper_dump_token(dumper, STR_LIT("token"), ast->AsmClobber.token);
		ast_dumper_dump_token(dumper, STR_LIT("name"), ast->AsmClobber.name);
		ast_dumper_dump_named_ast(dumper, STR_LIT("value"), ast->AsmClobber.value);
		break;
	case Ast_AsmLabelDecl:
		ast_dumper_dump_token(dumper, STR_LIT("token"), ast->AsmLabelDecl.token);
		ast_dumper_dump_named_ast(dumper, STR_LIT("name"), ast->AsmLabelDecl.name);
		break;
	case Ast_AsmInstruction:
		ast_dumper_dump_named_ast(dumper, STR_LIT("name"), ast->AsmInstruction.name);
		ast_dumper_dump_named_slice_of_asts(dumper, STR_LIT("operands"), ast->AsmInstruction.operands);
		ast_dumper_add_int_attribute(dumper, STR_LIT("valid_form_index"), ast->AsmInstruction.valid_form_index);
		break;
	case Ast_AsmMemoryOperand:
		{
			String mo_string = {};
			switch (ast->AsmMemoryOperand.kind) {
			case AsmMemoryOperand_Default: mo_string = STR_LIT("Default"); break;
			case AsmMemoryOperand_Pre: mo_string = STR_LIT("Pre"); break;
			case AsmMemoryOperand_Post: mo_string = STR_LIT("Post"); break;
			}
			ast_dumper_add_string_attribute(dumper, STR_LIT("kind"), mo_string);
		}
		ast_dumper_dump_token(dumper, STR_LIT("open"), ast->AsmMemoryOperand.open);
		ast_dumper_dump_named_ast(dumper, STR_LIT("segment_override"), ast->AsmMemoryOperand.segment_override);
		ast_dumper_dump_named_ast(dumper, STR_LIT("base"), ast->AsmMemoryOperand.base);
		ast_dumper_dump_token(dumper, STR_LIT("index_op"), ast->AsmMemoryOperand.index_op);
		ast_dumper_dump_named_ast(dumper, STR_LIT("index"), ast->AsmMemoryOperand.index);
		ast_dumper_dump_token(dumper, STR_LIT("scale_op"), ast->AsmMemoryOperand.scale_op);
		ast_dumper_dump_named_ast(dumper, STR_LIT("scale"), ast->AsmMemoryOperand.scale);
		ast_dumper_dump_token(dumper, STR_LIT("disp_op"), ast->AsmMemoryOperand.disp_op);
		ast_dumper_dump_named_ast(dumper, STR_LIT("disp"), ast->AsmMemoryOperand.disp);
		ast_dumper_dump_named_ast(dumper, STR_LIT("type"), ast->AsmMemoryOperand.type);
		ast_dumper_dump_token(dumper, STR_LIT("close"), ast->AsmMemoryOperand.close);
		break;
	case Ast_AsmRegisterGroup:
		ast_dumper_dump_token(dumper, STR_LIT("open"), ast->AsmRegisterGroup.open);
		ast_dumper_dump_named_slice_of_asts(dumper, STR_LIT("registers"), slice_from_array(ast->AsmRegisterGroup.registers));
		ast_dumper_dump_token(dumper, STR_LIT("range_token"), ast->AsmRegisterGroup.range_token);
		ast_dumper_dump_token(dumper, STR_LIT("close"), ast->AsmRegisterGroup.close);
		ast_dumper_dump_named_ast(dumper, STR_LIT("type"), ast->AsmRegisterGroup.type);
		break;
	case Ast_AsmDirective:
		ast_dumper_dump_token(dumper, STR_LIT("token"), ast->AsmDirective.token);
		ast_dumper_dump_token(dumper, STR_LIT("name"), ast->AsmDirective.name);
		ast_dumper_dump_named_slice_of_asts(dumper, STR_LIT("operands"), ast->AsmDirective.operands);
		break;
	case Ast_BadExpr:
