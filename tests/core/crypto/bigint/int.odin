...
internal_div_rem_u32 :: proc(t: ^testing.T) {
	for v in i31_div_rem_test_vectors {
		...
		quo, rem := _bigint._div_rem_u32(v.hi, v.lo, v.div)
...
@(test)
internal_i31_cond_negate :: proc(t: ^testing.T) {
	for v in i31_cond_negate_test_vectors {
		res: [12]u32
		copy(res[:], v.orig)

		_bigint._i31_cond_negate(res[:len(v.res)], v.ctl)
		testing.expect(t, slice.equal(res[:len(v.res)], v.res))

		if v.ctl == 1 {
			for i in 0..<len(v.orig) {
				testing.expect_value(t, v.orig[i] + res[i], 0x8000_0000 if i == 0 else 0x7fff_ffff)
			}
		}
	}
}
...
@(test)
internal_i31_finish_mod :: proc(t: ^testing.T) {
	for v in i31_finish_mod_test_vectors {
		res: [6]u32
		copy(res[:], v.orig)

		_bigint._i31_finish_mod(res[:], v.m, v.neg)
		testing.expect(t, slice.equal(res[:len(v.res)], v.res))
	}
}

@(test)
internal_i31_co_reduce :: proc(t: ^testing.T) {
	for v in i31_co_reduce_test_vectors {
		res_a: [6]u32 = 0
		res_b: [6]u32 = 0
		copy(res_a[:], v.orig_a)
		copy(res_b[:], v.orig_b)

		res := _bigint._i31_co_reduce(res_a[:4], res_b[:4], v.p[0], v.p[1], v.q[0], v.q[1])

		testing.expect_value(t, res, v.res)

		a_ok := slice.equal(res_a[:4], v.res_a[:4])
		if !a_ok {
			log.infof("\nres_a: %v\nv.res_a: %v", res_a, v.res_a)
		}

		b_ok := slice.equal(res_b[:4], v.res_b[:4])
		if !b_ok {
			log.infof("\nres_b: %v\nv.res_b: %v", res_b, v.res_b)
		}
...
@(test)
_internal_i31_co_reduce_mod :: proc(t: ^testing.T) {
	for v in i31_co_reduce_mod_test_vectors {
		res_a: [6]u32 = 0
		res_b: [6]u32 = 0
		copy(res_a[:], v.orig_a)
		copy(res_b[:], v.orig_b)

		m0i := _bigint.i31_ninv31(v.orig_a[0])
		_bigint._i31_co_reduce_mod(res_a[:4], res_b[:4], v.p[0], v.p[1], v.q[0], v.q[1], v.orig_a[:], m0i)
