...
stack_st_BIO          :: struct {}
sk_BIO_free_func      :: proc "c" (^BIO)
sk_BIO_copy_func      :: proc "c" (^BIO) -> ^BIO
sk_BIO_cmp_func       :: proc "c" (^^BIO, ^^BIO) -> i32
sk_BIO_delete_if_func :: proc "c" (^BIO, rawptr) -> i32
...
foreign lib {
	...
	BIO_read :: proc(bio: ^BIO, data: rawptr, len: i32) -> i32 ---
	...
	BIO_gets :: proc(bio: ^BIO, buf: cstring, size: i32) -> i32 ---

	// BIO_write writes |len| bytes from |data| to |bio|. It returns the number of
	// bytes written or a negative number on error.
	BIO_write :: proc(bio: ^BIO, data: rawptr, len: i32) -> i32 ---
	...
	BIO_ptr_ctrl :: proc(bp: ^BIO, cmd: i32, larg: c.long) -> cstring ---

	// BIO_int_ctrl acts like |BIO_ctrl| but passes the address of a copy of |iarg|
	// as |parg|.
	BIO_int_ctrl :: proc(bp: ^BIO, cmd: i32, larg: c.long, iarg: i32) -> c.long ---
	...
	BIO_eof :: proc(bio: ^BIO) -> i32 ---

	// BIO_set_flags ORs |flags| with |bio->flags|.
	BIO_set_flags :: proc(bio: ^BIO, flags: i32) ---

	// BIO_clear_flags ANDs |bio->flags| with the bitwise-complement of |flags|.
	BIO_clear_flags :: proc(bio: ^BIO, flags: i32) ---

	// BIO_test_flags returns |bio->flags| AND |flags|.
	BIO_test_flags :: proc(bio: ^BIO, flags: i32) -> i32 ---
	...
	// read.
	BIO_should_read :: proc(bio: ^BIO) -> i32 ---
	...
	// write.
	BIO_should_write :: proc(bio: ^BIO) -> i32 ---
	...
	BIO_should_retry :: proc(bio: ^BIO) -> i32 ---
	...
	BIO_should_io_special :: proc(bio: ^BIO) -> i32 ---
}
...
foreign lib {
	...
	BIO_get_retry_reason :: proc(bio: ^BIO) -> i32 ---
	...
	BIO_get_retry_flags :: proc(bio: ^BIO) -> i32 ---

	// BIO_clear_retry_flags clears the |BIO_FLAGS_READ|, |BIO_FLAGS_WRITE|,
	// |BIO_FLAGS_IO_SPECIAL| and |BIO_FLAGS_SHOULD_RETRY| flags from |bio|.
	BIO_clear_retry_flags :: proc(bio: ^BIO) ---

	// BIO_method_type returns the type of |bio|, which is one of the |BIO_TYPE_*|
	// values.
	BIO_method_type :: proc(bio: ^BIO) -> i32 ---
}

BIO_info_cb :: proc "c" (^BIO, i32, i32) -> i32
...
foreign lib {
	...
	BIO_callback_ctrl :: proc(bio: ^BIO, cmd: i32, fp: ^BIO_info_cb) -> c.long ---

	// BIO_pending returns the number of bytes pending to be read.
	BIO_pending :: proc(bio: ^BIO) -> c.size_t ---

	// BIO_ctrl_pending calls |BIO_pending| and exists only for compatibility with
	// OpenSSL.
	BIO_ctrl_pending :: proc(bio: ^BIO) -> c.size_t ---

	// BIO_wpending returns the number of bytes pending to be written.
	BIO_wpending :: proc(bio: ^BIO) -> c.size_t ---
	...
	BIO_push :: proc(bio: ^BIO, appended_bio: ^BIO) -> ^BIO ---
	...
	BIO_printf :: proc(bio: ^BIO, format: cstring, #c_vararg _: ..any) -> i32 ---

	// BIO_indent prints min(|indent|, |max_indent|) spaces. It returns one on
	// success and zero otherwise.
	BIO_indent :: proc(bio: ^BIO, indent: u32, max_indent: u32) -> i32 ---
	...
	BIO_hexdump :: proc(bio: ^BIO, data: ^u8, len: c.size_t, indent: u32) -> i32 ---

	// ERR_print_errors prints the current contents of the error stack to |bio|
	// using human readable strings where possible.
	ERR_print_errors :: proc(bio: ^BIO) ---
	...
	// from |bio|.
	BIO_read_asn1 :: proc(bio: ^BIO, out: ^^u8, out_len: ^c.size_t, max_len: c.size_t) -> i32 ---
}
...
foreign lib {
	...
	BIO_new_mem_buf :: proc(buf: rawptr, len: ossl_ssize_t) -> ^BIO ---

	// BIO_mem_contents sets |*out_contents| to point to the current contents of
	// |bio| and |*out_len| to contain the length of that data. It returns one on
	// success and zero otherwise.
	BIO_mem_contents :: proc(bio: ^BIO, out_contents: ^^u8, out_len: ^c.size_t) -> i32 ---
	...
	// empty.
	BIO_get_mem_data :: proc(bio: ^BIO, contents: ^cstring) -> c.long ---

	// BIO_get_mem_ptr sets |*out| to a BUF_MEM containing the current contents of
	// |bio|. It returns one on success or zero on error.
	BIO_get_mem_ptr :: proc(bio: ^BIO, out: ^^BUF_MEM) -> i32 ---
	...
	BIO_set_mem_buf :: proc(bio: ^BIO, b: ^BUF_MEM, take_ownership: i32) -> i32 ---
	...
	// |BIO_new_socket|).
	BIO_set_fd :: proc(bio: ^BIO, fd: i32, close_flag: i32) -> i32 ---
	...
	// This function may also be used with socket BIOs (see |BIO_s_socket| and
	// |BIO_new_socket|).
	BIO_get_fd :: proc(bio: ^BIO, out_fd: ^i32) -> i32 ---

	// BIO_s_file returns a BIO_METHOD that wraps a |FILE|.
	BIO_s_file :: proc() -> ^BIO_METHOD ---
...
foreign lib {
	...
	BIO_new_fp :: proc(file: ^FILE, flags: i32) -> ^BIO ---

	// BIO_get_fp sets |*out_file| to the current |FILE| for |bio|. It returns one
	// on success and zero otherwise.
	BIO_get_fp :: proc(bio: ^BIO, out_file: ^^FILE) -> i32 ---
	...
	// binary mode.
	BIO_append_filename :: proc(bio: ^BIO, filename: cstring) -> i32 ---

	// BIO_rw_filename opens |filename| for reading and writing and sets the result
	// as the |FILE| for |bio|. It returns one on success and zero otherwise. The
	// |FILE| will be closed when |bio| is freed. On Windows, the file is opened in
	// binary mode.
	BIO_rw_filename :: proc(bio: ^BIO, filename: cstring) -> i32 ---
	...
	BIO_seek     :: proc(bio: ^BIO, offset: c.long) -> c.long ---
	BIO_s_socket :: proc() -> ^BIO_METHOD ---
	...
	BIO_new_socket :: proc(fd: i32, close_flag: i32) -> ^BIO ---
	BIO_s_connect  :: proc() -> ^BIO_METHOD ---
	...
	BIO_new_connect :: proc(host_and_optional_port: cstring) -> ^BIO ---
	...
	BIO_set_conn_hostname :: proc(bio: ^BIO, host_and_optional_port: cstring) -> i32 ---
	...
	BIO_set_conn_port :: proc(bio: ^BIO, port_str: cstring) -> i32 ---

	// BIO_set_conn_int_port sets |*port| as the port that |bio| will connect to.
	// It returns one on success and zero otherwise.
	BIO_set_conn_int_port :: proc(bio: ^BIO, port: ^i32) -> i32 ---
...
BIO_CTRL_DGRAM_SET_MTU      :: 42 /* set cached value for  MTU. want to use
                                     this if asking the kernel fails */
BIO_CTRL_DGRAM_MTU_EXCEEDED :: 43 /* check whether the MTU was exceed in
...
foreign lib {
	...
	BIO_new_bio_pair :: proc(out1: ^^BIO, writebuf1: c.size_t, out2: ^^BIO, writebuf2: c.size_t) -> i32 ---

	// BIO_ctrl_get_read_request returns the number of bytes that the other side of
	// |bio| tried (unsuccessfully) to read.
	BIO_ctrl_get_read_request :: proc(bio: ^BIO) -> c.size_t ---
	...
	// |BIO_write| call.
	BIO_ctrl_get_write_guarantee :: proc(bio: ^BIO) -> c.size_t ---
	...
	BIO_meth_new :: proc(type: i32, name: cstring) -> ^BIO_METHOD ---

	// BIO_meth_free releases memory associated with |method|.
	BIO_meth_free :: proc(method: ^BIO_METHOD) ---
	...
	// error.
	BIO_meth_set_create :: proc(method: ^BIO_METHOD, create_func: proc "c" (^BIO) -> i32) -> i32 ---
	...
	BIO_meth_set_destroy :: proc(method: ^BIO_METHOD, destroy_func: proc "c" (^BIO) -> i32) -> i32 ---
	...
	BIO_meth_set_write :: proc(method: ^BIO_METHOD, write_func: proc "c" (^BIO, cstring, i32) -> i32) -> i32 ---

	// BIO_meth_set_read sets the implementation of |BIO_read| for |method| and
	// returns one.
	BIO_meth_set_read :: proc(method: ^BIO_METHOD, read_func: proc "c" (^BIO, cstring, i32) -> i32) -> i32 ---

	// BIO_meth_set_gets sets the implementation of |BIO_gets| for |method| and
	// returns one.
	BIO_meth_set_gets :: proc(method: ^BIO_METHOD, gets_func: proc "c" (^BIO, cstring, i32) -> i32) -> i32 ---

	// BIO_meth_set_ctrl sets the implementation of |BIO_ctrl| for |method| and
	// returns one.
	BIO_meth_set_ctrl :: proc(method: ^BIO_METHOD, ctrl_func: proc "c" (^BIO, i32, c.long, rawptr) -> c.long) -> i32 ---

	// BIO_meth_set_callback_ctrl sets the implementation of |BIO_callback_ctrl| for
	// |method| and returns one.
	BIO_meth_set_callback_ctrl :: proc(method: ^BIO_METHOD, callback_ctrl_func: proc "c" (^BIO, i32, ^BIO_info_cb) -> c.long) -> i32 ---
	...
	BIO_set_data :: proc(bio: ^BIO, ptr: rawptr) ---
...
foreign lib {
	...
	BIO_get_ex_new_index :: proc(argl: c.long, argp: rawptr, unused: ^CRYPTO_EX_unused, dup_unused: ^CRYPTO_EX_dup, free_func: ^CRYPTO_EX_free) -> i32 ---
	BIO_set_ex_data      :: proc(bio: ^BIO, idx: i32, arg: rawptr) -> i32 ---
	BIO_get_ex_data      :: proc(bio: ^BIO, idx: i32) -> rawptr ---
}
...
foreign lib {
	...
	BIO_f_base64          :: proc() -> ^BIO_METHOD ---
	BIO_set_retry_special :: proc(bio: ^BIO) ---

	// BIO_set_write_buffer_size returns zero.
	BIO_set_write_buffer_size :: proc(bio: ^BIO, buffer_size: i32) -> i32 ---

	// BIO_set_shutdown sets a method-specific "shutdown" bit on |bio|.
	BIO_set_shutdown :: proc(bio: ^BIO, shutdown: i32) ---

	// BIO_get_shutdown returns the method-specific "shutdown" bit.
	BIO_get_shutdown :: proc(bio: ^BIO) -> i32 ---

	// BIO_meth_set_puts returns one. |BIO_puts| is implemented with |BIO_write| in
	// BoringSSL.
	BIO_meth_set_puts :: proc(method: ^BIO_METHOD, puts: proc "c" (^BIO, cstring) -> i32) -> i32 ---
	...
	BIO_meth_get_write         :: proc(_: ^BIO, _: cstring, _: i32, method: ^BIO_METHOD) -> proc "c" (^BIO, cstring, i32) -> i32 ---
	BIO_meth_get_read          :: proc(_: ^BIO, _: cstring, _: i32, method: ^BIO_METHOD) -> proc "c" (^BIO, cstring, i32) -> i32 ---
	BIO_meth_get_gets          :: proc(_: ^BIO, _: cstring, _: i32, method: ^BIO_METHOD) -> proc "c" (^BIO, cstring, i32) -> i32 ---
	BIO_meth_get_puts          :: proc(_: ^BIO, _: cstring, method: ^BIO_METHOD) -> proc "c" (^BIO, cstring) -> i32 ---
	BIO_meth_get_ctrl          :: proc(_: ^BIO, _: i32, _: c.long, _: rawptr, method: ^BIO_METHOD) -> proc "c" (^BIO, i32, c.long, rawptr) -> c.long ---
	BIO_meth_get_create        :: proc(_: ^BIO, method: ^BIO_METHOD) -> proc "c" (^BIO) -> i32 ---
	BIO_meth_get_destroy       :: proc(_: ^BIO, method: ^BIO_METHOD) -> proc "c" (^BIO) -> i32 ---
	BIO_meth_get_callback_ctrl :: proc(_: ^BIO, _: i32, _: ^BIO_info_cb, method: ^BIO_METHOD) -> proc "c" (^BIO, i32, ^BIO_info_cb) -> c.long ---
}
...
BIO_TYPE_PROXY_CLIENT :: (14|BIO_TYPE_FILTER)
BIO_TYPE_PROXY_SERVER :: (15|BIO_TYPE_FILTER)
BIO_TYPE_NBIO_TEST    :: (16|BIO_TYPE_FILTER)
BIO_TYPE_NULL_FILTER  :: (17|BIO_TYPE_FILTER)
BIO_TYPE_BER          :: (18|BIO_TYPE_FILTER)       // BER -> bin filter
...
BIO_R_BAD_FOPEN_MODE          :: 100
BIO_R_BROKEN_PIPE             :: 101
BIO_R_CONNECT_ERROR           :: 102
BIO_R_ERROR_SETTING_NBIO      :: 103
BIO_R_INVALID_ARGUMENT        :: 104
BIO_R_IN_USE                  :: 105
BIO_R_KEEPALIVE               :: 106
BIO_R_NBIO_CONNECT_ERROR      :: 107
BIO_R_NO_HOSTNAME_SPECIFIED   :: 108
BIO_R_NO_PORT_SPECIFIED       :: 109
BIO_R_NO_SUCH_FILE            :: 110
BIO_R_NULL_PARAMETER          :: 111
BIO_R_SYS_LIB                 :: 112
BIO_R_UNABLE_TO_CREATE_SOCKET :: 113
BIO_R_UNINITIALIZED           :: 114
BIO_R_UNSUPPORTED_METHOD      :: 115
